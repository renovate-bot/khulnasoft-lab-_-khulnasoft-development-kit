...
### Edit the KhulnaSoft `gitlab.yml` file
...
1. Open `gitlab/config/gitlab.yml` and change the KhulnaSoft settings web server host line from either `localhost` or `127.0.0.1`  to reflect the IP of the previous step:
...
If you just want to
[monitor KhulnaSoft with Prometheus](https://docs.gitlab.com/ee/administration/monitoring/prometheus/) in
...
1. After Prometheus has started (on port `9090` by default), you can see the scrape status at
   `http://kdk.test:9090/targets`. After the `gitlab` target has been scraped once,
