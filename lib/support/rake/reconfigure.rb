    ...
    class Reconfigure
      ...
      def self.optional_tasks(config)
        [
          ...
          make_task('gitlab-elasticsearch-indexer-setup', enabled: config.elasticsearch?),
          make_task('gitlab-runner-setup', enabled: config.runner?),
          ...
          make_task('docs-gitlab-com-setup', enabled: config.docs_khulnasoft_com?),
          make_task('gitlab-observability-backend-setup', enabled: config.khulnasoft_observability_backend?),
          make_task('khulnasoft-k8s-agent-setup', enabled: config.khulnasoft_k8s_agent?),
          make_task('gitlab-pages-setup', enabled: config.khulnasoft_pages?),
          make_task('gitlab-ui-setup', enabled: config.khulnasoft_ui?),
